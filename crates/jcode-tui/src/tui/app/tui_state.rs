...
impl crate::tui::TuiState for App {
    ...
    fn selfdev_build_status(&self) -> Option<String> {
        ...
        let active = requests.iter().find(|r| r.session_id == self.session.id);
        active.map(|r| match r.state {
            jcode_app_core::tool::selfdev::BuildRequestState::Queued => {
                format!("Build queued ({})", r.reason)
            }
            jcode_app_core::tool::selfdev::BuildRequestState::Building => r
                .last_progress
                .clone()
